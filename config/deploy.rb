set :application, 'nataliehopson'
set :repo_url, 'git@github.com:dandanv/nataliehopson.git'
set :theme_name, 'nataliehopson'
...
# set :linked_dirs, fetch(:linked_dirs, []).push('web/app/wflogs')
...
namespace :assets do
  ...
  task :sagecomposer do
    on roles(:web) do
      set :theme_path_root, -> { release_path.join(fetch(:theme_path)) }

      within fetch(:theme_path_root) do
        info " Running composer install on sage theme "
        execute :composer, :install
      end
    end
  end

  task deploy: %w(compile copy sagecomposer)
end
...
after 'deploy:publishing', 'assets:deploy'
